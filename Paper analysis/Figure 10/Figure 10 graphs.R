...
############################################################### Vcmd ################################################################
file_path3 <- paste0(file_path2, '/Raw ABF data summaries/', identifier)
setwd(file_path3)
...
# see Figure 7 raw data processing.R
# $`NPY Cre X iSPN WT`$`23d20000` gain off by factor of 20 
Vcmd$`NPY Cre X iSPN WT`[4] <- Vcmd$`NPY Cre X iSPN WT`[4]/20

Vcmd_ordered <- c(
  Vcmd[['NPY Cre X dSPN WT']],
  Vcmd[['NPY Cre X iSPN WT']],
  Vcmd[['NPY Cre X dSPN 6OHDA']],
  Vcmd[['NPY Cre X iSPN 6OHDA']]
)

Vcmd_SPN <- cbind(area_SPN[, c('s', 'condition', 'cell_type')], Vcmd = Vcmd_ordered)
...
MCwilcox(formula=Vcmd~cell_type*condition, df=Vcmd_SPN)


BoxPlot3(formula= Vcmd~cell_type*condition, data=Vcmd_SPN[-1], wid=wid, cap=cap, xlab='', ylab = ylab, xrange=xrange, 
  yrange=yrange, xlabel_angle=45, tick_length=tick_length, y_tick_interval=y_tick_interval, lwd=lwd, type=type, amount=amount, 
